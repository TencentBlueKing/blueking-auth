...
func Test_oauthDeviceCodeManager_Approve(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		mock.ExpectExec(
			`^UPDATE oauth_device_code SET status = 'approved', tenant_id = \?, sub = \?, username = \?, audience = \? WHERE id = \? AND status = 'pending'$`,
		).
			WithArgs(
				"default",
				"user1",
...
func Test_oauthDeviceCodeManager_SlowDown(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		mock.ExpectExec(
			`^UPDATE oauth_device_code SET poll_interval = poll_interval \+ \?, last_polled_at = NOW\(\) WHERE id = \?$`,
		).
			WithArgs(
				int64(5),
				int64(1),
