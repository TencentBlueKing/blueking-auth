...
type OAuthDeviceCodeManager interface {
	...
	SlowDown(ctx context.Context, id, increment int64) (int64, error)
}
...
// Create inserts a device authorization row and returns the new row ID.
...
// GetByDeviceCode loads a device authorization record by device_code.
...
// GetByUserCode loads a device authorization record by user_code.
...
// UpdateStatus sets the status column for the row identified by id.
...
// Approve marks a pending device code as approved and binds tenant and user identity.
...
// ConsumeApproved marks an approved, non-expired row as consumed for the given client.
...
// UpdateLastPolledAt sets last_polled_at to the current time for the row.
...
// SlowDown increases poll_interval and refreshes last_polled_at (RFC 8628 slow_down).
func (m *oauthDeviceCodeManager) SlowDown(ctx context.Context, id, increment int64) (int64, error) {
