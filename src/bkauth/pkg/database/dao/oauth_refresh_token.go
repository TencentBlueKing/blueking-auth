...
// CreateWithTx inserts a refresh token row within the given transaction.
...
// GetByTokenHash loads a refresh token by hashed value; returns a zero value when missing.
...
// RevokeWithTx marks the refresh token as revoked by id within the transaction.
...
// RevokeByGrantIDWithTx revokes all non-revoked refresh tokens sharing the grant ID.
