...
// Create inserts an OAuth client row.
...
// Get loads a client by ID; returns a zero value when no row exists.
...
// GetGrants returns redirect URI and grant type configuration for the client.
...
// GetDisplay returns presentation fields for the client (name, type, logo URI).
