...
func NewTokenHandler(cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.ShouldBind(&req); err != nil {
			c.JSON(http.StatusBadRequest, oauth.NewInvalidRequestError("Missing or invalid parameters"+err.Error()))
			return
		}
...
func handleAuthorizationCodeGrant(c *gin.Context, cfg *config.Config, req TokenRequest) {
	...
	authCode, err := authCodeSvc.ValidateAndConsume(ctx, realmName, req.Code, clientID, req.RedirectURI, req.CodeVerifier)
	if err != nil {
