...
func webJSONSuccess(c *gin.Context, data any) {
	c.JSON(http.StatusOK, webSuccessResponse{Data: data})
}

func webJSONError(c *gin.Context, httpStatus int, code, message string) {
...
func webJSONErrorWithDetails(c *gin.Context, httpStatus int, code, message string, details []webErrorDetail) {
	c.JSON(httpStatus, webErrorResponse{
		Error: webError{
			Code:       code,
			Message:    message,
