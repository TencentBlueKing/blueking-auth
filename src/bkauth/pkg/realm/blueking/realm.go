...
// Package blueking provides the OAuth realm implementation for BlueKing gateway MCP servers and API resources.
package blueking
...
func parseBluekingResource(item string) (resType, name, apiName string, err error) {
	if strings.HasPrefix(item, "http://") || strings.HasPrefix(item, "https://") {
		...
		if n == "" {
			return "", "", "", fmt.Errorf(
				"invalid resource URL: cannot extract MCP server name from %q",
				item,
			)
		}
		return "mcp", n, "", nil
	...
	if strings.HasPrefix(item, "gateway:") {
		...
		if apiIdx < 0 {
			return "", "", "", fmt.Errorf(
				"invalid resource: gateway item must contain :api: segment in %q",
				item,
			)
		}
		gwName := rest[:apiIdx]
...
// ValidateResource checks that each comma-separated entry uses a supported blueking resource format.
...
// ExtractAudiences derives deduplicated OAuth audience strings from the realm resource list.
...
// ResolveResourceDisplay builds structured display data for consent UI, including MCP titles when available.
