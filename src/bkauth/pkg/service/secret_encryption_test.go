...
var _ = Describe("secretEncryptionService", func() {
	Describe("ListEncryptionStatus cases", func() {
		...
		It("ok", func() {
			...
			mockManager.EXPECT().List().Return(
				[]dao.AccessKey{
			...
			result, err := svc.ListEncryptionStatus()
		...
		It("manager fail", func() {
			mockManager := mock.NewMockAccessKeyManager(ctl)
			mockManager.EXPECT().List().Return(nil, errors.New("db fail"))

			svc := secretEncryptionService{accessKeyManager: mockManager}
			_, err := svc.ListEncryptionStatus()
	...
	Describe("ReencryptLegacySecret cases", func() {
		...
		It("rewrites only legacy rows", func() {
			...
			mockManager.EXPECT().
				UpdateByID(int64(2), map[string]interface{}{"app_secret": "enc:secret2"}).
				Return(int64(1), nil)

			svc := secretEncryptionService{accessKeyManager: mockManager}
			result, err := svc.ReencryptLegacySecret()
			assert.NoError(GinkgoT(), err)
		...
		It("reports rows already rewritten when update fails", func() {
			...
			mockManager.EXPECT().UpdateByID(int64(1), gomock.Any()).Return(int64(1), nil)
			mockManager.EXPECT().UpdateByID(int64(2), gomock.Any()).Return(int64(0), errors.New("db fail"))
