...
type SecretEncryptionService interface {
	ListEncryptionStatus() ([]AccessKeyEncryptionStatus, error)
	ReencryptLegacySecret() ([]AccessKeyEncryptionStatus, error)
}
...
func (s *secretEncryptionService) ListEncryptionStatus() ([]AccessKeyEncryptionStatus, error) {
	errorWrapf := errorx.NewLayerFunctionErrorWrapf(SecretEncryptionSVC, "ListEncryptionStatus")

	daoAccessKeys, err := s.accessKeyManager.List()
	if err != nil {
	...
	for _, daoAccessKey := range daoAccessKeys {
		legacy, err := IsLegacyEncryptedAppSecret(daoAccessKey.AppSecret)
		if err != nil {
			return nil, errorWrapf(err, "IsLegacyEncryptedAppSecret accessKeyID=`%d` fail", daoAccessKey.ID)
		}
...
func (s *secretEncryptionService) ReencryptLegacySecret() ([]AccessKeyEncryptionStatus, error) {
	...
	for _, daoAccessKey := range daoAccessKeys {
		legacy, err := IsLegacyEncryptedAppSecret(daoAccessKey.AppSecret)
		if err != nil {
			return reencrypted, errorWrapf(err, "IsLegacyEncryptedAppSecret accessKeyID=`%d` fail", daoAccessKey.ID)
		}
		if !legacy {
			continue
		}
		...
		if err != nil {
			return reencrypted, errorWrapf(err, "ConvertToPlainAppSecret accessKeyID=`%d` fail", daoAccessKey.ID)
		}

		encryptedSecret, err := ConvertToEncryptedAppSecret(plainSecret)
		if err != nil {
			return reencrypted, errorWrapf(err, "ConvertToEncryptedAppSecret accessKeyID=`%d` fail", daoAccessKey.ID)
		}

		_, err = s.accessKeyManager.UpdateByID(daoAccessKey.ID, map[string]interface{}{
