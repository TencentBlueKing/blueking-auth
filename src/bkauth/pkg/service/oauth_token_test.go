...
var _ = Describe("oauthTokenService", func() {
	...
	Describe("prepareTokenPair", func() {
		It("should build a token pair with hashed tokens and encoded audience", func() {
			...
			Expect(
				prepared.daoAccessToken.ExpiresAt,
			).To(
				BeTemporally("~", start.Add(5*time.Minute), 2*time.Second),
			)
			...
			Expect(
				prepared.daoRefreshToken.ExpiresAt,
			).To(
				BeTemporally("~", start.Add(time.Hour), 2*time.Second),
			)
		...
		It("should revoke old tokens and persist a rotated pair", func() {
			...
			mockAccessManager.EXPECT().
				CreateWithTx(
					gomock.Any(),
					gomock.Any(),
					gomock.AssignableToTypeOf(dao.OAuthAccessToken{}),
				).
				DoAndReturn(
					func(_ context.Context, _ *sqlx.Tx, token dao.OAuthAccessToken) (int64, error) {
			...
			mockRefreshManager.EXPECT().
				CreateWithTx(
					gomock.Any(),
					gomock.Any(),
					gomock.AssignableToTypeOf(dao.OAuthRefreshToken{}),
				).
				DoAndReturn(
					func(_ context.Context, _ *sqlx.Tx, token dao.OAuthRefreshToken) (int64, error) {
			...
			pair, err := svc.RefreshAccessToken(
				context.Background(),
				"blueking",
				"refresh-1",
				"client-1",
