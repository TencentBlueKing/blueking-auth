...
type OAuthDeviceCodeService interface {
	...
	PollAndConsumeDeviceCode(
		ctx context.Context,
		realmName, deviceCode, clientID string,
...
// CreateDeviceCode generates device and user codes and stores a pending device authorization request.
...
// GetByUserCode returns pending device authorization details for a normalized user code.
...
// ApproveByUserCode records the authenticated user and audience on the pending device authorization.
func (s *oauthDeviceCodeService) ApproveByUserCode(
	...
	if _, err := s.deviceCodeManager.Approve(
		ctx,
		dc.ID,
		tenantID,
		sub,
...
// DenyByUserCode rejects the pending device authorization associated with the user code.
