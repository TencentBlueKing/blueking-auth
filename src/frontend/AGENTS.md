...
蓝鲸认证 & TOKEN 管理前端（BlueKing Auth）—— 基于 Vue 3 + TypeScript 的单页应用，用于管理用户认证和 TOKEN。位于 `blueking-auth` 仓库的 `src/frontend/` 目录下。
...
| HTTP 请求  | Axios                        | ^1            |
| 主 UI 库   | bkui-vue (蓝鲸 UI)           | 2.0.2-beta.93 |
| 表格       | tdesign-vue-next（高级表格） | ^1.18         |
| 原子化 CSS | UnoCSS + Tailwind v4 预设    | ^66.3         |
| 工具库     | @vueuse/*                    | ^14           |
...
- **`main.ts`** — 创建 Vue 应用，安装 Pinia、Vue Router、bkui-vue（全量引入）、vue-i18n、XSS 过滤指令，注册全局组件 `CommonIcon`、
...
- **全局组件**（在 `main.ts` 注册，无需导入直接使用）：`CommonIcon`、`IconButton`、`CopyButton`、`CardContainer`
...
- **UnoCSS + Tailwind v4 预设**：原子化 CSS 工具类（如 `flex items-center gap-16px`）
...
7. **index.html & index.dev.html & index.prod.html**: package.json 中有个脚本会运行 replace-index-html.js 替换 index.html 中的内容，这是预期内的行为，不需要去检查、比较、恢复 index.html 中的内容
